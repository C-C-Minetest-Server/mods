 ...
 return function(pos, node, asp)
	if not asp.main.free then
		if asp.shunt.free then
			advtrains.ndb.swap_node(pos, {name="groundline:signal_shunt", param2 = node.param2})
		else
			advtrains.ndb.swap_node(pos, {name="groundline:signal_danger", param2 = node.param2})
		end
	else
		if asp.dst.free and asp.main.speed == -1 then
			advtrains.ndb.swap_node(pos, {name="groundline:signal_free", param2 = node.param2})
		else
			advtrains.ndb.swap_node(pos, {name="groundline:signal_slow", param2 = node.param2})
		end
	end
...
for typ, prts in pairs({
	...
	minetest.register_node("groundline:signal_"..typ, {
...
minetest.register_node("groundline:tcb_node", {
	drawtype = "mesh",
	paramtype="light",
	paramtype2="facedir",
	walkable = false,
